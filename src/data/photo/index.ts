...
// Raw db insert
export type PhotoDbInsert = PhotoExif & {
    id: string;
    url: string;
    extension: string;
    blurData: string;
...
export const parsePhotoFromDb = (photoDbRaw: PhotoDb): Photo => {
    ...
    return {
        ...
        takenAtNaiveFormatted: formatDateFromPostgresString(
            photoDb.takenAtNaive
        ),
        takenAtNaiveFormattedShort: formatDateFromPostgresString(
            photoDb.takenAtNaive,
            true
        ),
    };
...
export const dateRangeForPhotos = (photos: Photo[]) => {
    ...
    const description = start === end ? start : `${start}–${end}`;
