...
async function getFirebaseFileURL(filename) {
   const storageRef = ref(storage, filename);
   const url = storageRef.getDownloadURL();
   await downloadFileFromURL(url, `./${filename}`);
}

export function getFireBaseConfigData() {
   const projectsData = getFirebaseFileURL(PROJECTS_DATA);
   const homeData = getFirebaseFileURL(HOME_DATA);
   return { projectsData, homeData };
}
