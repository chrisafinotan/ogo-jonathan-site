...
async function getFileURLFirebase(filename) {
    const storageRef = await ref(storage, `assets/${filename}`);
   const url = await getDownloadURL(storageRef);
   return url;
} 

async function getandDownloadFileURLFirebase(filename) {
   const storageRef = await ref(storage, `assets/${filename}`);
   const url = await getDownloadURL(storageRef);
...
async function listFolderFilesFirebase(foldername) {
   const storageRef = await ref(storage, `assets/${foldername}`);
...
export async function getFireBaseConfigData(files = []) {
   ...
   const res = await Promise.all(files.map((path) => getandDownloadFileURLFirebase(path)));
   return res;
}
...
export async function getAllHomeProjects() {
   ...
   for (const project in data) {
      let projectObj = {
         ...
         content: await getFileURLFirebase(data[project].File),
         name: data[project].Name,
